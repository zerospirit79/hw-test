...
class HWTestConfig:
    ...
    def __post_init__(self):
        ...
        self.colors_enabled = self.color_mode == "always" or (
            self.color_mode == "auto" and os.isatty(1)
        )
    ...
    def _load_file(self, path: Path) -> None:
        ...
        try:
            ...
            if 'settings' in config:
                ...
                self.config.verbose = settings.getboolean(
                    'verbose', self.config.verbose)
                self.config.batch_mode = settings.getboolean(
                ...
                self.config.local_media_base = settings.get('local_media_base', self.config.local_media_base)
                self.config.local_media_check = settings.get('local_media_check', self.config.local_media_check)
                self.config.ping_server = settings.get('ping_server', self.config.ping_server)
                self.config.express_video_set = settings.get('express_video_set', self.config.express_video_set)
                self.config.local_video_sample = settings.get('local_video_sample', self.config.local_video_sample)
                ...
                if 'skip_tests' in settings:
                    self.config.skip_tests = [
                        t.strip() for t in settings['skip_tests'].split(',')
                    ]
