...
class UpgradeStep(BaseHWStep):
    ...
    def _run_command(
        self, cmd: List[str], timeout: int = 300, use_root: bool = True
    ) -> Tuple[str, str, int]:
    ...
    def _dist_upgrade(self) -> Tuple[bool, str]:
        ...
        stdout, stderr, rc = self._run_command(["apt-get", "dist-upgrade", "-y"], timeout=600)
        if rc != 0:
            return False, stderr
        return True, ""
    ...
    def _update_kernel(self) -> Tuple[bool, str]:
        ...
        stdout, stderr, rc = self._run_command(["update-kernel", "-f"], timeout=300)
        if rc != 0:
            return False, stderr
        return True, ""
    ...
    def _install_packages(self, packages: List[str]) -> Tuple[bool, str]:
        ...
        stdout, stderr, rc = self._run_command(["apt-get", "install", "-y"] + packages, timeout=600)
        if rc != 0:
            return False, stderr
        return True, ""
