...
class PrepareStep(BaseHWStep):
    ...
    def _check_kernel_errors(self) -> Optional[str]:
        ...
        if rc == 0:
            ...
            if len(matches) > 9:
                return (
                    "Too many AER errors. Use pcie_aspm=off, pci=nomsi or pci=noaer boot options."
                )
        return None
    ...
    def _detect_distro(self) -> Dict[str, str]:
        ...
        try:
            ...
            # Check for ALT SP
            if (
                "alt 8 sp" in distro_name_lower
                or "alt sp" in distro_name_lower
                or "alt 8 sp" in pretty_name_lower
                or "alt sp" in pretty_name_lower
                or "(cliff)" in distro_name_lower
                or "(cliff)" in pretty_name_lower
            ):
